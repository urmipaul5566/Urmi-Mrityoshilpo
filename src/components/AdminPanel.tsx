...
export default function AdminPanel({ onNavigate }: AdminPanelProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        // Compress and convert file to web-friendly optimized JPEG base64
  ...
  // Real Cloud Database State & Backups
  const [dbStatusInfo, setDbStatusInfo] = useState<any>(null);
  const [backupLogs, setBackupLogs] = useState<string[]>(["[System] Connected to Google Firebase Firestore Cloud Database."]);
  ...
  const handleBackupDb = async () => {
    setBackingUp(true);
    setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Fetching live backup from cloud...`, ...prev]);
    try {
      const token = localStorage.getItem("urmi_token") || localStorage.getItem("clay_admin_token") || "";
      const res = await fetch("/api/admin/database/backup", {
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (!res.ok) throw new Error("ব্যাকআপ ডাউনলোড করা সম্ভব হয়নি");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `urmi_clay_cloud_backup_${Date.now()}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      setBackupLogs((prev) => [
        `[${new Date().toLocaleTimeString()}] Live Cloud Backup downloaded successfully!`,
        ...prev
      ]);
    } catch (err: any) {
      alert("ব্যাকআপ ডাউনলোড করতে সমস্যা হয়েছে: " + err.message);
    } finally {
      setBackingUp(false);
    }
  };
  ...
  const handleSyncDb = async () => {
    setSyncingDb(true);
    setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Syncing with Firebase Cloud Firestore...`, ...prev]);
    try {
      await api.admin.syncDatabase();
      await loadDbStatus();
      setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Cloud database sync successfully verified!`, ...prev]);
      alert("ক্লাউড ডাটাবেজ (Firebase Firestore) সফলভাবে সিঙ্ক করা হয়েছে!");
    } catch (err: any) {
      setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Sync error: ${err.message}`, ...prev]);
  ...
  const handleRestoreDefaults = async () => {
    if (!confirm("আপনি কি নিশ্চিত যে আপনি ডিফল্ট ডাটাতে রিস্টোর করতে চান? পূর্ববর্তী সকল ডাটা রিফ্রেশ হয়ে যাবে।")) return;
    setRestoringDb(true);
    setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Restoring database defaults to cloud Firestore...`, ...prev]);
    try {
      await api.admin.restoreDefaults();
      await loadDbStatus();
      await loadAdminData();
      setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Database restored to initial default state successfully!`, ...prev]);
      alert("ডাটাবেজ সফলভাবে ডিফল্ট অবস্থায় রিস্টোর করা হয়েছে!");
    } catch (err: any) {
      setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Restore error: ${err.message}`, ...prev]);
      alert("রিস্টোর ব্যর্থ হয়েছে: " + err.message);
    } finally {
  ...
  const handleRestoreBackupFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!confirm("আপনি কি নিশ্চিত যে আপনি এই ব্যাকআপ ফাইলটি রিস্টোর করতে চান? বর্তমান ডাটাবেজ এই ফাইলের ডাটা দিয়ে প্রতিস্থাপিত হবে।")) {
      e.target.value = "";
      return;
    }
    setRestoringDb(true);
    setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Reading backup file: ${file.name}...`, ...prev]);
    try {
      const text = await file.text();
      const backupData = JSON.parse(text);
      await api.admin.restoreBackup(backupData);
      await loadDbStatus();
      await loadAdminData();
      setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Database restored successfully from backup file ${file.name}!`, ...prev]);
      alert("ব্যাকআপ ফাইল থেকে ডাটাবেজ সফলভাবে রিস্টোর করা হয়েছে!");
    } catch (err: any) {
      setBackupLogs((prev) => [`[${new Date().toLocaleTimeString()}] Restore backup error: ${err.message}`, ...prev]);
      alert("ব্যাকআপ রিস্টোর ব্যর্থ হয়েছে: " + err.message);
    } finally {
      setRestoringDb(false);
      e.target.value = "";
    }
          ...
          <div className="lg:col-span-9 flex flex-col gap-6">
            ...
            {activeTab === "db-backup" && (
              <div className="bg-white/70 p-6 rounded-3xl border border-clay-secondary/10 shadow-sm flex flex-col gap-6 font-bangla text-xs">
                <div>
                  <h4 className="text-base font-bold text-clay-primary mb-2 flex items-center gap-2">
                    <Database className="w-5 h-5 text-green-600" />
                    ক্লাউড ডাটাবেজ এবং ব্যাকআপ ব্যবস্থাপনা (Cloud Database & Backup Engine)
                  </h4>
                  <p className="text-gray-500 leading-relaxed">
                    আপনার ই-কমার্স স্টোরের সকল পণ্য, অর্ডার এবং গ্রাহক ডাটা রিয়েল-টাইম <strong>Google Firebase Firestore Cloud Database</strong>-এ সংরক্ষিত ও সিঙ্ক হচ্ছে। নিচে ডাটাবেসের বর্তমান অবস্থা ও বিকল্প সমাধানসমূহ দেখুন।
                  </p>
                </div>

                {/* Status Dashboard Card */}
                {dbStatusInfo ? (
                  <div className="bg-emerald-50/70 border border-emerald-300 rounded-2xl p-5 flex flex-col gap-3">
                    <div className="flex items-center justify-between border-b border-emerald-200 pb-3">
                      <div>
                        <span className="bg-green-600 text-white font-sans text-[10px] uppercase font-bold px-2.5 py-1 rounded-full">
                          {dbStatusInfo.connected ? "Connected & Active ✅" : "Local Fallback"}
                        </span>
                        <h5 className="font-bold text-gray-800 text-sm mt-1.5 font-sans">{dbStatusInfo.provider}</h5>
                      </div>
                      <div className="text-right font-sans text-xs text-gray-600">
                        <p><strong>Project ID:</strong> {dbStatusInfo.projectId}</p>
                        <p><strong>Total Records:</strong> {dbStatusInfo.totalRecords} items</p>
                      </div>
                    </div>

                    {/* Collection Counts Grid */}
                    <div className="grid grid-cols-4 sm:grid-cols-7 gap-2 pt-1 font-sans text-center">
                      {dbStatusInfo.collections && Object.entries(dbStatusInfo.collections).map(([col, count]) => (
                        <div key={col} className="bg-white rounded-xl p-2 border border-emerald-100 shadow-2xs">
                          <p className="text-[10px] text-gray-400 uppercase font-semibold">{col}</p>
                          <p className="text-sm font-bold text-emerald-700">{String(count)}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ) : (
                  <div className="p-4 bg-gray-50 rounded-xl text-gray-500">ডাটাবেজ স্ট্যাটাস লোড হচ্ছে...</div>
                )}
                ...
                <div className="flex flex-wrap gap-4">
                  <button
                    ...
                    disabled={backingUp}
                    className="bg-clay-primary hover:bg-clay-secondary text-white font-bold px-5 py-3 rounded-xl transition-all shadow flex items-center gap-2 text-sm"
                  >
                    <Database className="w-4.5 h-4.5" />
                    {backingUp ? "ডাউনলোড হচ্ছে..." : "ডাটা ব্যাকআপ নিন (Backup Now)"}
                  </button>

                  <button
                    onClick={handleRestoreDefaults}
                    disabled={restoringDb}
                    className="bg-amber-600 hover:bg-amber-700 text-white font-bold px-5 py-3 rounded-xl transition-all shadow flex items-center gap-2 text-sm"
                  >
                    <RefreshCw className={`w-4.5 h-4.5 ${restoringDb ? "animate-spin" : ""}`} />
                    {restoringDb ? "রিস্টোর হচ্ছে..." : "ডাটা রিস্টোর করুন (Restore Defaults)"}
                  </button>

                  <label className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-5 py-3 rounded-xl transition-all shadow flex items-center gap-2 cursor-pointer text-sm">
                    <Upload className="w-4.5 h-4.5" />
                    {restoringDb ? "আপলোড হচ্ছে..." : "ফাইল থেকে রিস্টোর (Restore JSON)"}
                    <input
                      type="file"
                      accept=".json"
                  ...
                  <button
                    ...
                    disabled={syncingDb}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-5 py-3 rounded-xl transition-all shadow flex items-center gap-2 text-sm"
                  >
                    <RotateCcw className={`w-4.5 h-4.5 ${syncingDb ? "animate-spin" : ""}`} />
                    {syncingDb ? "সিঙ্ক হচ্ছে..." : "ফায়ারস্টোর লাইভ সিঙ্ক (Live Cloud Sync)"}
                  </button>
                </div>

                {/* Alternative Database Options Explained */}
                <div className="border-t border-clay-secondary/10 pt-5">
                  <h5 className="font-bold text-clay-primary mb-3 text-sm">অন্যান্য অপশন ও ডাটাবেজ সমাধান (Alternative Database Options)</h5>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {dbStatusInfo?.alternativeOptions?.map((opt: any, idx: number) => (
                      <div key={idx} className="bg-white p-4 rounded-2xl border border-gray-200 shadow-2xs flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-start mb-1.5">
                            <h6 className="font-bold text-gray-800 text-sm font-sans">{opt.name}</h6>
                            <span className="text-[10px] bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-semibold">{opt.status}</span>
                          </div>
                          <p className="text-[11px] text-gray-400 font-sans mb-2">{opt.type}</p>
                ...
                {/* Logs */}
                <div className="border-t border-clay-secondary/10 pt-5">
                  <h5 className="font-bold text-clay-secondary mb-3 uppercase tracking-wider">সিস্টেম ও সিঙ্ক লগ (Database Log)</h5>
                  <div className="bg-[#222222] text-green-400 p-4 rounded-2xl font-mono text-[11px] h-40 overflow-y-auto space-y-1.5">
